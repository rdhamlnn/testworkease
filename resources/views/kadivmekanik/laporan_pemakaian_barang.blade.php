    ...
    function updateTable(data) {
        ...
        if (data && data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<button type="button" class="btn btn-warning btn-sm btn-edit" data-id="' + item.id_laporan_pemakaian_barang + '" data-toggle="modal" data-target="#editBarangModal"><i class="fas fa-edit"></i></button>';
                if (item.id_laporan_pemakaian_barang) {
                    html += '<button type="button" class="btn btn-danger btn-sm btn-delete" data-url="/kadivmekanik/laporan-pemakaian-barang/' + item.id_laporan_pemakaian_barang + '" data-message="Yakin ingin menghapus laporan ini?"><i class="fas fa-trash"></i></button>';
                }
                html += '</div>';
                html += '</td>';
                html += '</tr>';
