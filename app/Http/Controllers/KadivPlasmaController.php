...
class KadivPlasmaController extends Controller
{
    ...
    {
        $userDivisiNama = DB::table('divisi')->where('id_divisi', Session::get('user_divisi'))->value('nama_divisi') ?? 'Plasma';
        ...
        $nextWorkOrderNumber = $this->generateWorkOrderNumber('PLS');
        $daftarBarang = \App\Models\DaftarBarang::orderBy('nama_barang')->get();
        $divisiPengaju = $userDivisiNama;

        return view('kadivplasma.work_order', compact('userDivisiNama', 'divisiTujuan', 'unit', 'jenisWorkOrder', 'nextWorkOrderNumber', 'workOrders', 'daftarBarang', 'divisiPengaju'));
    }
    ...
    public function riwayatWorkOrder()
    {
        ...
        return view('kadivplasma.riwayat_work_order', compact('workOrders', 'daftarBarang'));
    }
    ...
    public function storeWorkOrder(Request $request)
    {
        ...
        try {
            ...
            $workOrder = SuratPengajuan::create([
                'no_surat_pengajuan' => $this->generateWorkOrderNumber('PLS'),
            ...
            return redirect()->route('kadivplasma.work-order')->with('error', 'Gagal: ' . $e->getMessage());
        }
    }
